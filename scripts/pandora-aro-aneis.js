// Script descartável — para peças de ANEL do leilão Pandora, converte o tamanho
// europeu (sufixo após a barra em Descricao_2, ex: "-2/50" → 50) em aro brasileiro
// e adiciona " - Aro N" no final dos campos Descricao e Peca.
//
// Tabela de conversão europeu → aro BR:
//   48→10  50→12  52→14  54→16  56→18  58→20
//
// Uso:  node --env-file=.env scripts/pandora-aro-aneis.js 64491
//       node --env-file=.env scripts/pandora-aro-aneis.js 64491 --limit=3   (testa só 3 peças)
//       node --env-file=.env scripts/pandora-aro-aneis.js 64491 --dry-run   (só mostra, não grava)
//
...
const EURO_PARA_ARO = { '48': 10, '50': 12, '52': 14, '54': 16, '56': 18, '58': 20 };
...
if (!numLeilao) {
  console.error('Uso: node --env-file=.env scripts/pandora-aro-aneis.js <numLeilao> [--limit=N] [--skip=N] [--dry-run]');
  process.exit(1);
}
...
// Descricao_2 no formato "codigo-variacao/tamanhoEuropeu", ex: "190017C01-2/50"
function extrairAro(descricao2) {
  const m = descricao2.match(/\/(\d{2})$/);
  if (!m) return null;
  const euro = m[1];
  return EURO_PARA_ARO[euro] ?? null;
}
...
async function main() {
  ...
  let ok = 0, jaTinha = 0, naoEhAnel = 0, semTamanho = 0, erro = 0;
  ...
  for (const { pieceId } of pecas) {
    try {
      ...
      const pecaAtual = (fields.get('Peca') || '').trim();

      if (!/anel/i.test(descricaoAtual)) {
        naoEhAnel++;
        continue;
      }

      const aroTag = / - Aro \d+$/;
      if (aroTag.test(descricaoAtual) || aroTag.test(pecaAtual)) {
        console.log(`  [peça ${pieceId}] já tem aro: "${descricaoAtual}" — pulando`);
        jaTinha++;
        continue;
      }

      const aro = extrairAro(descricao2);
      if (aro === null) {
        console.log(`  [peça ${pieceId}] não consegui extrair tamanho de "${descricao2}" — pulando`);
        semTamanho++;
        continue;
      }

      const novaDescricao = `${descricaoAtual} - Aro ${aro}`;
      const novaPeca       = `${pecaAtual} - Aro ${aro}`;
      console.log(`  [peça ${pieceId}] "${descricaoAtual}" → "${novaDescricao}" | Peça: "${pecaAtual}" → "${novaPeca}"`);
  ...
  console.log('\n─── Resumo ───');
  console.log(`OK: ${ok}`);
  console.log(`Já tinham aro (puladas): ${jaTinha}`);
  console.log(`Não é anel (puladas): ${naoEhAnel}`);
  console.log(`Sem tamanho reconhecido: ${semTamanho}`);
