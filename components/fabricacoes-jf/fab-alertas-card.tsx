...
const STYLES: Record<Alerta['status_alerta'], { row: string; badge: string }> = {
  RUPTURA: { row: 'bg-semantic-red/10 border-l-4 border-l-semantic-red', badge: 'bg-semantic-red text-white' },
  CRITICO: { row: 'bg-semantic-amber/10 border-l-4 border-l-semantic-amber', badge: 'bg-semantic-amber text-white' },
  ...
  return (
    <Card variant="bordered">
      ...
      <div className="p-3 flex flex-col gap-2 max-h-80 overflow-y-auto">
        {alertas.length === 0 ? (
          <div className="py-8 text-center text-text-muted text-sm">✅ Nenhum alerta no momento</div>
        ) : (
          alertas.map((a, i) => (
            <div key={i} className={`flex items-center justify-between p-3 rounded-lg gap-3 text-sm ${STYLES[a.status_alerta].row}`}>
              <div>
                <div className="font-semibold text-text">{a.subtipo}</div>
