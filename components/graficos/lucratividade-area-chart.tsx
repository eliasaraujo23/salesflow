...
function DataLabel({ x = 0, y = 0, value = 0 }: CustomLabelProps) {
  ...
  const W = Math.max(text.length * 6.5 + 16, 60);
  const H = 22;
  return (
    <g>
...
export function LucratividadeAreaChart({ data }: Props) {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={chartData} margin={{ top: 40, right: 70, left: 8, bottom: 0 }}>
        ...
        />
        <Tooltip
          formatter={(v) => [fmtPct(Number(v ?? 0)), 'Lucratividade']}
        ...
        <Area
          ...
          activeDot={{ r: 6 }}
        >
          <LabelList dataKey="lucratividade" content={DataLabel as any} />
        </Area>
      </AreaChart>
    </ResponsiveContainer>
  );
