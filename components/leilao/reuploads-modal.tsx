        ...
        <div className="shrink-0 px-5 py-4 border-t border-zinc-200 dark:border-white/[0.08] flex flex-col gap-3">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            {state.phase === 'idle' && (
              <button
                onClick={onExecute}
                className="flex items-center gap-1.5 px-4 py-2 text-xs rounded-lg bg-violet-600 hover:bg-violet-700 text-white font-semibold transition-colors"
              >
                <Camera size={12} />
                Executar — {pecas.length} peça{pecas.length !== 1 ? 's' : ''}
              </button>
            )}
          </div>
        </div>
