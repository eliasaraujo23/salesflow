    ...
    <header className="h-topbar bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-white/[0.13] px-3 md:px-6 flex items-center justify-between shrink-0 gap-2">
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {/* Inline tabs for /fabricacoes-jm */}
          {pathname.startsWith('/fabricacoes-jm') && (
            <div className="flex items-center gap-1 ml-1">
              <ChevronRight size={14} className="text-zinc-400 shrink-0" />
              {[
                { label: 'Estoque',     href: '/fabricacoes-jm' },
              ].map(tab => {
          ...
          {/* Inline tabs for /fabricacoes-jc */}
          {pathname.startsWith('/fabricacoes-jc') && (
            <div className="flex items-center gap-1 ml-1">
              <ChevronRight size={14} className="text-zinc-400 shrink-0" />
              {[
                { label: 'Estoque', href: '/fabricacoes-jc' },
              ].map(tab => {
                const isActive = pathname === tab.href || pathname.startsWith(tab.href + '/');
                return (
                  <Link
