...
function downloadCSV(rows: MaintenanceItem[]) {
  const hdr = ['Ref','Tipo','Produto','Subtipo','Pedra','Lapidação','Dest. Manut.','Destino','Saída Manut.','Dias','Custo'];
  const esc = (v: string | number | null | undefined) => `"${String(v ?? '').replace(/"/g, '""')}"`;
  const csv = [
    hdr.join(','),
    ...rows.map(r => [
      r.referencia, r.tipo, r.produto, r.subtipo, r.tipo_pedra, r.lapidacao,
      r.destino_manutencao, r.destino, fmtDate(r.data_saida_manutencao), r.dias,
  ...
  a.href = url; a.download = 'manutencoes.csv'; a.click();
  URL.revokeObjectURL(url);
}

type StringField = 'tipo' | 'produto' | 'subtipo' | 'destino_manutencao' | 'destino' | 'tipo_pedra' | 'lapidacao';

function uniqStrings(rows: MaintenanceItem[], field: StringField): string[] {
...
function applyDimFilters(
  ...
  destManut: string[], destinos: string[], pedras: string[], lapidacoes: string[],
): MaintenanceItem[] {
...
export function MaintenanceTab({ data }: MaintenanceTabProps) {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <button className="flex items-center gap-1 text-[11px] font-semibold text-zinc-500 dark:text-zinc-400" onClick={() => column.toggleSorting()}>
          Saída Manut. <SortIcon dir={column.getIsSorted()} />
        </button>
      ),
      cell: ({ getValue }) => (
        <span className="text-xs text-zinc-500 dark:text-zinc-400 whitespace-nowrap">
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/[0.06] rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-200 dark:border-white/[0.06]">
        <span className="text-[13px] font-semibold text-zinc-900 dark:text-zinc-100">Peças em Manutenção</span>
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 text-[11px] font-medium text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-white/[0.06] rounded hover:border-emerald-500 hover:text-emerald-600 transition-colors disabled:opacity-40"
          >
      ...
      <div className="overflow-x-auto">
        ...
        {filtered.length === 0 && (
          <div className="p-10 text-center text-zinc-500 dark:text-zinc-400 text-sm">
            {data.length === 0 ? 'Nenhuma peça em manutenção' : 'Nenhum item para os filtros selecionados'}
          </div>
        )}
      </div>
    </div>
