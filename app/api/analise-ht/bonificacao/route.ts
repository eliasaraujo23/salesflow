...
import { calcularBonificacao, DEFAULT_BONIFICACAO_PARAMS, type AnaliseHtRegistroDb } from '@/lib/analise-ht/bonificacao';

const paramsSchema = z.object({
  uploadIds: z.array(z.string().min(1)).min(1),
  teorMedio: z.number().min(0).max(1).default(DEFAULT_BONIFICACAO_PARAMS.teorMedio),
  valorFino: z.number().min(0).default(DEFAULT_BONIFICACAO_PARAMS.valorFino),
  percentual: z.number().min(0).max(1).default(DEFAULT_BONIFICACAO_PARAMS.percentual),
  limitePagoPorGrama: z.number().min(0).default(DEFAULT_BONIFICACAO_PARAMS.limitePagoPorGrama),
});
...
export async function POST(req: NextRequest) {
  ...
  for (const uploadId of uploadIds) {
    ...
    const porAvaliador = calcularBonificacao(registros, params);
