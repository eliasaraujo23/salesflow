...
import { useConversaoJm } from '@/hooks/use-conversao-jm';
...
export default function PainelConversaoJmPage() {
  const { from, to } = usePainelFilters();
  const { data, isLoading, isError, error } = useConversaoJm(from, to);
      ...
      {data && !isLoading && (
        <>
          ...
          <div className="shrink-0 h-56 overflow-auto bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/[0.08] rounded-xl">
            ...
            {sortedRows.length === 0 && (
              <div className="text-center py-12 text-zinc-400 text-sm">Nenhuma peça modificada no período.</div>
            )}
          </div>
        </>
      )}
