...
export default function PainelMercadoLivrePage() {
  const { from, to } = usePainelFilters();
  const { data, isLoading, isError, error } = usePainelVendas(from, to, null);

  const mlRows = useMemo(() => {
    if (!data) return [];
    return data.rows.filter(r => r.destino === 'MERCADO LIVRE');
  }, [data]);

  const kpi = useMemo(() => {
    const fat = mlRows.reduce((s, r) => s + r.preco_cobrado, 0);
    const custo = mlRows.reduce((s, r) => s + r.custo_real, 0);
    const qtd = mlRows.length;
  ...
  const byTipo = useMemo((): TipoEntry[] => {
    ...
    return tipos.flatMap(tipo => {
      const rows = mlRows.filter(r => r.tipo === tipo);
  ...
  return (
    <div className="h-full overflow-hidden p-4 flex flex-col gap-3">
      ...
      {data && !isLoading && mlRows.length === 0 && (
        <div className="flex-1 flex items-center justify-center text-zinc-400 text-sm">
          Nenhuma venda no Mercado Livre no período selecionado.
        </div>
      )}
      ...
      {data && !isLoading && mlRows.length > 0 && (
        <>
          ...
          <div className="flex-1 min-h-0 grid gap-3" style={{ gridTemplateColumns: `repeat(${Math.max(byTipo.length, 1)}, 1fr)` }}>
            {byTipo.map(t => (
              <div key={t.tipo} className={`bg-white dark:bg-zinc-900 border rounded-xl overflow-hidden flex flex-col transition-colors ${selectedTipo === t.tipo ? 'border-amber-400 dark:border-amber-500' : 'border-zinc-200 dark:border-white/[0.08]'}`}>
                  ...
                  <th colSpan={9} className="sticky top-0 z-20 px-4 py-3 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-white/[0.08] text-left">
                    <div className="flex items-center gap-3">
                      ...
                      {(selectedTipo || selectedProduto) && (
                        <button onClick={clearFilter} className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-semibold hover:bg-amber-100 transition-colors">
