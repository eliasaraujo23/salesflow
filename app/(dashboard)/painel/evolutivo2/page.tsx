...
export default function PainelEvolutivo2Page() {
  const [selectedYears, setSelectedYears] = useState<Set<string>>(new Set());
  const [activeCanais, setActiveCanais] = useState<Set<string>>(new Set(['parceiros', 'eternno', 'leiloes', 'mercadolivre']));
  ...
  const { years, monthlyRows } = useMemo(() => {
    if (!rawData) return { years: [] as string[], monthlyRows: [] as Record<string, number | string>[] };
    ...
    return { years: [...yearSet].sort(), monthlyRows: rows };
  }, [rawData]);
    ...
    <div className="h-full overflow-auto p-4 flex flex-col gap-3">
      ...
      {rawData && !isLoading && (
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-white/[0.08] rounded-xl p-4 flex flex-col gap-2">
          <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">Evolução Mensal por Canal</p>
